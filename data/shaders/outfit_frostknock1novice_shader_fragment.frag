...
// Função para detectar bordas e criar aura azul usando textura de nuvem
float blueAura(vec2 uv, sampler2D tex, sampler2D cloudTex, vec2 cloudUV, float time) {
...
void main()
{
    ...
    // Cor azul NEON muito clara e brilhante
    vec3 divineBlue = vec3(0.4, 0.8, 1.0); // Azul claro neon
    vec3 brightBlue = vec3(0.5, 0.9, 1.0); // Azul neon brilhante para sparkles
    vec3 sparkleBlue = vec3(0.45, 0.85, 1.0); // Azul neon sparkle
    vec3 auraBlue = vec3(0.4, 0.8, 1.0); // Azul neon para aura
    ...
    // Gera aura azul ao redor das bordas usando textura stone
    float auraValue = blueAura(v_TexCoord, u_Tex0, u_Tex1, v_TexCoord3, u_Time);
    
    // Aplica cor azul baseada nas cores do outfit
    if(texcolor.r > 0.9) {
        // Vermelho -> Azul divino brilhante
        vec4 outfitColor = texcolor.g > 0.9 ? u_Color[0] : u_Color[1];
        vec3 blueTint = divineBlue * pulse;
        
        // Adiciona sparkles espalhados (mistura mais suave)
        vec3 sparkleColor = mix(blueTint, sparkleBlue, sparkleValue * 0.5);
        sparkleColor = mix(sparkleColor, brightBlue, sparkleValue * 0.3);
        ...
        // Azul -> Azul divino mais intenso
        vec4 outfitColor = u_Color[3];
        vec3 blueTint = vec3(0.35, 0.75, 1.0) * pulse; // Azul neon mais intenso para azul
        
        // Adiciona sparkles espalhados
        vec3 sparkleColor = mix(blueTint, sparkleBlue, sparkleValue * 0.4);
        sparkleColor = mix(sparkleColor, brightBlue, sparkleValue * 0.25);
        
        gl_FragColor.rgb = gl_FragColor.rgb * outfitColor.rgb * sparkleColor;
        gl_FragColor.a = gl_FragColor.a * outfitColor.a;
    } else {
        // Outras áreas -> Aplica azul divino com sparkles espalhados
        vec3 blueTint = divineBlue * pulse;
        vec3 sparkleColor = mix(blueTint, sparkleBlue, sparkleValue * 0.6);
        sparkleColor = mix(sparkleColor, brightBlue, sparkleValue * 0.4);
    ...
    // Adiciona um brilho espalhado e difuso nas áreas com sparkles (azul neon)
    gl_FragColor.rgb += sparkleValue * vec3(0.1, 0.3, 0.4);
    ...
    // Aplica efeitos azuis apenas onde há conteúdo do personagem
    // Adiciona brilho azul com padrão stone nas bordas internas
    if (auraValue > 0.0 && gl_FragColor.a > 0.1) {
        vec3 auraColor = auraBlue * auraValue * (0.5 + stoneIntensity * 0.5);
    ...
    if (stoneIntensity > 0.2 && gl_FragColor.a > 0.1) {
        vec3 stoneGlow = auraBlue * stoneIntensity * 0.3;
