...
*/
DllExport void scale(size_t factor, //valid range: 2 - 6
           const uint32_t* src, uint32_t* trg, int srcWidth, int srcHeight,
           ColorFormat colFmt,
           const ScalerCfg& cfg = ScalerCfg(),
           int yFirst = 0, int yLast = (std::numeric_limits<int>::max)()); //slice of source image

DllExport void nearestNeighborScale(const uint32_t* src, int srcWidth, int srcHeight,
...
//parameter tuning
DllExport bool equalColorTest(uint32_t col1, uint32_t col2, ColorFormat colFmt, double luminanceWeight, double equalColorTolerance);
