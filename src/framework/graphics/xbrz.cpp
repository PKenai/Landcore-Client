...
{
    yFirst = std::max<int>(yFirst, 0);
    yLast  = std::min<int>(yLast, srcHeight);
    ...
    if (yFirst > 0)
    {
        ...
        const uint32_t* s_m1 = src + srcWidth * std::max<int>(y - 1, 0);
        const uint32_t* s_0  = src + srcWidth * y; //center line
        const uint32_t* s_p1 = src + srcWidth * std::min<int>(y + 1, srcHeight - 1);
        const uint32_t* s_p2 = src + srcWidth * std::min<int>(y + 2, srcHeight - 1);

        for (int x = 0; x < srcWidth; ++x)
        {
            const int x_m1 = std::max<int>(x - 1, 0);
            const int x_p1 = std::min<int>(x + 1, srcWidth - 1);
            const int x_p2 = std::min<int>(x + 2, srcWidth - 1);
    ...
    for (int y = yFirst; y < yLast; ++y)
    {
        ...
        for (int x = 0; x < srcWidth; ++x, out += Scaler::scale)
        {
            ...
            const int x_m1 = std::max<int>(x - 1, 0); //perf: prefer array indexing to additional pointers!
            const int x_p1 = std::min<int>(x + 1, srcWidth - 1);
            const int x_p2 = std::min<int>(x + 2, srcWidth - 1);
...
struct ColorDistanceARGB
{
    static double dist(uint32_t pix1, uint32_t pix2, double luminanceWeight)
    {
        ...
        */
        ...
        //return std::min<int>(a1, a2) * DistYCbCrBuffer::dist(pix1, pix2) + 255 * abs(a1 - a2);
                                ...
                                SliceType st, int yFirst, int yLast)
{
    ...
    switch (st)
    {
        ...
        case NN_SCALE_SLICE_TARGET:
            ...
            yFirst = std::max<int>(yFirst, 0);
            yLast  = std::min<int>(yLast, trgHeight);
