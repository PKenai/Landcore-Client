...
struct DrawQueueItemTexturedRect : public DrawQueueItem {
  ...
  DrawQueueItemTexturedRect(const Rect &dest, const TexturePtr &texture,
                            const Rect &src, const Color &color)
...
struct DrawQueueItemTextureCoords : public DrawQueueItem {
  DrawQueueItemTextureCoords(CoordsBuffer &coordsBuffer,
                             const TexturePtr &texture, const Color &color)
      : DrawQueueItem(texture, color),
        m_coordsBuffer(std::move(coordsBuffer)) {};
...
struct DrawQueueItemColoredTextureCoords : public DrawQueueItem {
  DrawQueueItemColoredTextureCoords(
      CoordsBuffer &coordsBuffer, const TexturePtr &texture,
      const std::vector<std::pair<int, Color>> &colors)
      : DrawQueueItem(texture), m_coordsBuffer(std::move(coordsBuffer)),
        m_colors(colors) {};
...
struct DrawQueueItemImageWithShader : public DrawQueueItemTextureCoords {
  DrawQueueItemImageWithShader(CoordsBuffer &coords, const TexturePtr &texture,
...
struct DrawQueueItemFilledRectWithShader : public DrawQueueItem {
  DrawQueueItemFilledRectWithShader(const Rect &rect, const Color &color,
...
struct DrawQueueItemText : public DrawQueueItem {
  DrawQueueItemText(const Point &point, const TexturePtr &texture,
                    uint64_t hash, const Color &color, bool shadow = false)
...
struct DrawQueueItemTextColored : public DrawQueueItem {
  DrawQueueItemTextColored(const Point &point, const TexturePtr &texture,
                           uint64_t hash,
                           const std::vector<std::pair<int, Color>> &colors,
                           bool shadow = false)
...
struct DrawQueueItemLine : public DrawQueueItem {
  DrawQueueItemLine(const std::vector<Point> &points, int width,
                    const Color &color)
...
struct DrawQueueCondition {
  DrawQueueCondition(size_t start, size_t end) : m_start(start), m_end(end) {}
...
struct DrawQueueConditionRotation : public DrawQueueCondition {
  DrawQueueConditionRotation(size_t start, size_t end, const Point &center,
                             float angle)
...
struct DrawQueueConditionFlipHorizontal : public DrawQueueCondition {
  DrawQueueConditionFlipHorizontal(size_t start, size_t end,
                                   const Point &center)
...
public:
  ...
  addTexturedRect(const Rect &dest, const TexturePtr &texture, const Rect &src,
                  const Color &color = Color::white) {
    DrawQueueItemTexturedRect *item(
        new DrawQueueItemTexturedRect(dest, texture, src, color));
    m_queue.push_back(item);
    return item;
  }
  void addTextureCoords(CoordsBuffer &coords, const TexturePtr &texture,
                        const Color &color = Color::white) {
    m_queue.push_back(new DrawQueueItemTextureCoords(coords, texture, color));
  }
  void
  addColoredTextureCoords(CoordsBuffer &coords, const TexturePtr &texture,
                          const std::vector<std::pair<int, Color>> &colors) {
    m_queue.push_back(
        new DrawQueueItemColoredTextureCoords(coords, texture, colors));
  }
  ...
  void addText(BitmapFontPtr font, const std::string &text,
               const Rect &screenCoords,
               Fw::AlignmentFlag align = Fw::AlignTopLeft,
               const Color &color = Color::white, bool shadow = false);
  void addColoredText(BitmapFontPtr font, const std::string &text,
  ...
  void addBoundingRect(const Rect &dest, int innerLineWidth,
                       const Color &color = Color::white) {
  ...
  void addLine(const std::vector<Point> &points, int width,
               const Color &color = Color::white) {
  ...
  bool hasFrameBuffer() { return m_useFrameBuffer; }
  Rect getFrameBufferDest() { return m_frameBufferDest; }
  Size getFrameBufferSize() { return m_frameBufferSize; }
  Rect getFrameBufferSrc() { return m_frameBufferSrc; }
  ...
  void setClip(size_t start, const Rect &clip) {
    ...
    m_conditions.push_back(
        new DrawQueueConditionClip(start, m_queue.size(), clip));
  }

  void setRotation(size_t start, const Point &center, float angle) {
    if (start == m_queue.size() || angle == 0)
      return;
    m_conditions.push_back(
        new DrawQueueConditionRotation(start, m_queue.size(), center, angle));
  }
  ...
  void setMark(size_t start, const Color &color) {
    if (start == m_queue.size())
      return;
    m_conditions.push_back(
        new DrawQueueConditionMark(start, m_queue.size(), color));
  }
  ...
  void setFlipHorizontal(size_t start, const Point &center) {
    if (start == m_queue.size())
      return;
    m_conditions.push_back(
        new DrawQueueConditionFlipHorizontal(start, m_queue.size(), center));
  }
